...
## Some new shortcuts added by Hanxiao
### Normal mode
`space y` copy word under cursor to system clipboard

`space yy` copy line under cursor to system clipboard

`n space yy` copy n lines under cursor to system clipboard

`space u` update diff in vimdiff

`space space o` only keep current window in vim

`space /` turn off highlights for search

`space q` :q

`space j` last cursor position

`space k` next cursor position

`space ]` go to tag with new tab

`space \` go to tag with new split

`space n` open nerd tree

`space f` search word under cursor

`space g` return to the position before search

`space .` comment current line

`space ,` uncomment current line

`space o` open url or file under cursor

`ctrl e` show syntax check errors

`f [hjkl]` move hjkl window inside vim

### Insert mode
`space tab` snip trigger

`jj` snip jump forward 

`kk` snip jump backward

### Visual mode
`space y` copy selected words
